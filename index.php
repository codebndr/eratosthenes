...
function cache_file()
{
    ...
    $cache_dir = '/tmp/codebenderPhpcache';

    return $cache_dir . $cache_key;
}

function update_cache($stringData)
{
...
function read_cache()
{
    ...
    $contents = fread($fh, filesize(cache_file()));
    fclose($fh);
...
{
    return (date("U") - filemtime(cache_file())) > 600;
}

if (file_exists(cache_file()) && !cache_stale()) {
    echo read_cache();
} else {
...
// Instantiate the class
	...
	if ($data == "included") {
		...
		if ($data == "list") {
			...
			$response["categories"] = array(
			    "Examples" => $handler->fetchBuiltinInfo(),
			    "Builtin Libraries" => $handler->fetchInfo('libraries/'),
			    "External Libraries" => $handler->fetchInfo(
				'external-libraries/'
			    )
			);
		    } else {
    ...
    update_cache(json_encode($response));
}
...
class LibraryHandler
{
    ...
    {
	$response = $this->s3->get_object_list(
	    $this->bucket,
	    array('prefix' => 'examples', 'pcre' => '/\.ino/')
	);
	...
	$response = $this->s3->get_object_list(
	    $this->bucket,
	    array('prefix' => 'libraries', 'pcre' => '/\.ino/')
	);
	...
	$response = $this->s3->get_object_list(
	    $this->bucket,
	    array('prefix' => 'external-libraries', 'pcre' => '/(\.ino|\.pde)/i')
	);
	...
	$response = $this->s3->get_object_list(
	    $this->bucket,
	    array('prefix' => 'examples/', "delimiter" => "/")
	);
	...
	$response = $this->s3->get_object_list(
	    $this->bucket,
	    array('prefix' => 'libraries/', "delimiter" => "/")
	);
	foreach ($response as $key => $value) {
	    $array = explode("/", $value);
	    $list[] = $array[1];
	...
	$response = $this->s3->get_object_list(
	    $this->bucket,
	    array('prefix' => 'external-libraries/', "delimiter" => "/")
	);
	$list = array();
	foreach ($response as $key => $value) {
	    $array = explode("/", $value);
	...
	foreach ($list as $library) {
	    ...
	    $response = $this->s3->get_object_list(
		$this->bucket,
		array('prefix' => $library . "/", 'pcre' => '/(\.ino|\.pde)/i')
	    );
	...
	foreach ($list as $library) {
	    ...
	    $response = $this->s3->get_object_list(
		$this->bucket,
		array('prefix' => $library . "/examples", 'pcre' => '/(\.ino|\.pde)/i')
	    );
    ...
    {
	$list = $this->s3->get_object_list(
	    $this->bucket,
	    array('prefix' => $library . "/", "delimiter" => "/", 'pcre' => '/(README\.)/i')
	);
	$description = "none";
